...
function setupSecurityControls() {
  ...
  window.addEventListener("blur", () => {
    ...
    if (now - lastBlurAt > 1500) {
      ...
      showSecurityWarning("שימו לב: המערכת זיהתה יציאה מחלון המטלה. האירוע יישמר עם ההגשה.");
    }
  });
  ...
  document.addEventListener("fullscreenchange", () => {
    if (!document.fullscreenElement) {
      ...
      showSecurityWarning("יצאת ממסך מלא. מומלץ לחזור למסך מלא כדי להשלים את המטלה.");
    }
  });
  ...
  ["copy", "cut", "paste"].forEach(eventName => {
    document.addEventListener(eventName, event => {
      ...
      showSecurityWarning("פעולת העתקה/הדבקה נרשמה במערכת.");
    });
  });
